...
//----------- jQuery.ajax()
	...
	ajaxTest( "jQuery.ajax() - responseText on error", 1, {
		...
		error: function( xhr ) {
			strictEqual( xhr.responseText, "plain text message", "Test jqXHR.responseText is selected for HTTP errors" );
		}
	});
...
//----------- jQuery.fn.load()
	...
	asyncTest( "jQuery.fn.load( String, Function ) - dataFilter in ajaxSettings", 2, function() {
		...
		jQuery("<div/>").load( url("data/name.html"), function( responseText ) {
			strictEqual( jQuery( this ).html(), "Hello World", "Test div was selected with filtered data" );
